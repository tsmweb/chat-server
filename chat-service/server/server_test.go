...
func TestServer(t *testing.T) {
	...
	conn1 := newConnUser(t, ln.Addr().String(), repository.UserTest1)
	conn2 := newConnUser(t, ln.Addr().String(), repository.UserTest2)
	defer func() {
		conn1.Close()
		conn2.Close()
...
func initServer(t *testing.T) *server.Server {
	...
	handleUserStatus := server.NewHandleUserStatus(userEncoder, kaf.NewProducer(config.KafkaUsersTopic()), repository)
